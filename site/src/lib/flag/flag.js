...
import { FLAG_ICONS } from '$lib/flag/icons.js';
...
export function flagIcon(k, cx, cy, size, fill, field) {
  const s = size / 512;
...
export function politiskelFlag(c, p) {
...
export function drawFlag({ layout, cols, syms: given, rainbow = false, border = false, wide = false }) {
  ...
  if (layout === "diagonal") {
    ...
    /* The layout already says revolution, so the triangle goes to the
       profile's own revolutionary symbol when it has one — a star repeating
       the triangle is then dropped — and keeps the star otherwise. */
    const claim = syms.find(s => FLAG_REBEL.has(s.symbol)) || syms.find(s => s.symbol === "star");
    const star = !claim;
...
export function flagLegend(t, drawn, layout, shown, border, rainbow, starOfLayout) {
