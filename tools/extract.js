...
function readAnswers() {
  ...
  for (const file of fs.readdirSync(ANSWERS).filter(f => /\.json$/i.test(f)).sort()) {
    ...
    out.push({ alias: data.alias.trim(), source: typeof data.source === "string" ? data.source : null,
               answers });
    console.log("  + answers/" + file.padEnd(24) + data.alias.trim().padEnd(9)
...
function main() {
  ...
  /* Answers that name a screenshot must find it here: exported from a page
     built with other screenshots, they would otherwise turn into a second,
     questionnaire-only profile under the same alias. */
  for (const a of answers) {
    if (a.source && !profiles.some(p => p.source === a.source))
      console.log("  ? answers for " + a.alias + " name " + a.source
        + ", which is not in politi-results/ — kept as a questionnaire-only profile");
  }
  ...
  console.log("\n" + fresh + " extracted, " + reused + " cached, " + failed + " failed"
    + (answers.length ? ", " + answers.length + " answer file(s)" : "")
